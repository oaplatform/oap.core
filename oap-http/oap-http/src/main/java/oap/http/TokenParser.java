...
public class TokenParser {
    ...
    public String parseToken( CharSequence buf, ParserCursor cursor, BitSet delimiters ) {
        Preconditions.checkNotNull( buf, "Char sequence" );
        Preconditions.checkNotNull( cursor, "Parser cursor" );
    ...
    public void copyContent( CharSequence buf, ParserCursor cursor, BitSet delimiters,
                             StringBuilder dst ) {
        Preconditions.checkNotNull( buf, "Char sequence" );
        Preconditions.checkNotNull( cursor, "Parser cursor" );
        Preconditions.checkNotNull( dst, "String builder" );
        int pos = cursor.getPos();
        int indexFrom = cursor.getPos();
        int indexTo = cursor.getUpperBound();
        for( int i = indexFrom; i < indexTo; i++ ) {
     ...
     */
    @SuppressWarnings( "checkstyle:UnnecessaryParentheses" )
    public void copyUnquotedContent( CharSequence buf, ParserCursor cursor,
                                     BitSet delimiters, StringBuilder dst ) {
        Preconditions.checkNotNull( buf, "Char sequence" );
        Preconditions.checkNotNull( cursor, "Parser cursor" );
        Preconditions.checkNotNull( dst, "String builder" );
        int pos = cursor.getPos();
     ...
     * @param dst    destination buffer
     */
    public void copyQuotedContent( CharSequence buf, ParserCursor cursor,
                                   StringBuilder dst ) {
        Preconditions.checkNotNull( buf, "Char sequence" );
        Preconditions.checkNotNull( cursor, "Parser cursor" );
        Preconditions.checkNotNull( dst, "String builder" );
